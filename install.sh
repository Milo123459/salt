...
get_tmpfile() {
  ...
  suffix="$1"
  if has mktemp; then
    printf "%s%s.%s.%s" "$(mktemp)" "-salt" "${RANDOM}" "${suffix}"
  else
    # No really good options here--let's pick a default + hope
    printf "/tmp/salt.%s" "${suffix}"
  fi
}
...
download() {
  ...
  printf "\n" >&2
  info "This is likely due to salt not yet supporting your configuration."
  info "If you would like to see a build for your configuration,"
  info "please create an issue requesting a build for ${MAGENTA}${TARGET}${NO_COLOR}:"
  info "${BOLD}${UNDERLINE}https://github.com/Milo123459/salt/issues/new/${NO_COLOR}"
  return $rc
}
...
unpack() {
  ...
  info "bug report at https://github.com/Milo123459/salt/issues"
  return 1
}
...
install() {
  ...
  if test_writeable "${BIN_DIR}"; then
    sudo=""
    msg="Installing salt, please wait…"
  else
    warn "Escalated permissions are required to install to ${BIN_DIR}"
    elevate_priv
    sudo="sudo"
    msg="Installing salt as root, please wait…"
  fi
  info "$msg"
...
is_build_available() {
  ...
  if [ "${good}" != "1" ]; then
    error "${arch} builds for ${platform} are not yet available for salt"
    printf "\n" >&2
    info "If you would like to see a build for your configuration,"
    info "please create an issue requesting a build for ${MAGENTA}${target}${NO_COLOR}:"
    info "${BOLD}${UNDERLINE}https://github.com/Milo123459/salt/issues/new/${NO_COLOR}"
...
UNINSTALL=0
HELP=0
CARGOTOML="$(curl -fsSL https://raw.githubusercontent.com/Milo123459/salt/master/Cargo.toml)"
...
if [ -z "${BASE_URL-}" ]; then
  BASE_URL="https://github.com/Milo123459/salt/releases"
fi
...
if [ $UNINSTALL == 1 ]; then
  confirm "Are you sure you want to uninstall salt?"
  ...
  if test_writeable "$(dirname "$(which salt)")"; then
    sudo=""
    msg="Removing salt, please wait…"
  else
    warn "Escalated permissions are required to install to ${BIN_DIR}"
    elevate_priv
    sudo="sudo"
    msg="Removing salt as root, please wait…"
  fi
...
URL="${BASE_URL}/latest/download/salt-${TARGET}.${EXT}"
info "Tarball URL: ${UNDERLINE}${BLUE}${URL}${NO_COLOR}"
confirm "Install salt ${GREEN}${VERSION[0]}${NO_COLOR} to ${BOLD}${GREEN}${BIN_DIR}${NO_COLOR}?"
check_bin_dir "${BIN_DIR}"

install "${EXT}"
completed "salt installed"